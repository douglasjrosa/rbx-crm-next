...
export default function CompanyLogo ( { displayName, website, size }: {
	displayName: string
	website?: string
	size: number
} ) {
	...
	const cleanDomain = website?.replace( /(.*:\/\/)|(www.)|(\/.*)/g, "" )
	const logoUrl = cleanDomain ? `https://logo.clearbit.com/${ cleanDomain }` : ""
	...
	console.log( initialsClasses )
	
	const defaultClasses = `rounded-full size-[${ size }px] flex items-center justify-center`

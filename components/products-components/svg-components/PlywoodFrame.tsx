...
export default function PlywoodFrame ( {
	...
	return (
		<div className="relative rounded shadow-cover bg-white dark:opacity-80 pt-3 w-full flex flex-wrap" >
				...
				<svg width={ viewboxWidth * scale } height={ viewboxHeight * scale } viewBox={ viewBox } >
					...
					<g id="frame" >
						{ pieces.map( ( piece, index ) => {
							...
							const { name } = piece as SelectedPieceType
							const pieceIndex = Number( name.replace( /\D/g, "" ) ) - 1

							return (
								<rect
									className={ `${ name } hover:fill-[#48ff00] active:fill-[#ffff00]` }
									onClick={ ( e ) => {
										e.stopPropagation()
										!!setSelectedPiece && setSelectedPiece( { name, pieceIndex, x, y, width, height } )
									} }
									key={ `piece-${ index }` }
									x={ x }
									y={ y }
									width={ width }
									height={ height }
									fill={ name === selectedPiece?.name ? "#007dff" : fill }
									stroke="black"
								/>
							)
						} ) }
					...
					{ !!stickers && stickers.length > 0 && !!plywoodThickness &&
						<Stickers x={ x0 } y={ y0 } stickers={ stickers } gaps={ gaps } />
					}
